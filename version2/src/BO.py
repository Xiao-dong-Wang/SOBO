...
class BO:
    ...
    def optimize_constr(self, x):
        ...
        try:
            fmin_l_bfgs_b(loss, x0, approx_grad=True, bounds=[[-0.5,0.5]]*x.size, maxiter=2000, m=100, iprint=self.debug, callback=callback)
        except np.linalg.LinAlgError:
            print('Optimizing constrains. Increase noise term and re-optimization')
            x0 = np.copy(best_x).reshape(-1)
            x0[0] += 0.01
            try:
                fmin_l_bfgs_b(loss, x0, approx_grad=True, bounds=[[-0.5,0.5]]*x.size, maxiter=2000, m=10, iprint=self.debug, callback=callback)
            except:
