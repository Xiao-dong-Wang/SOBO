from optimparallel import fmin_l_bfgs_b_parallel
import numpy as np
import time
...
class GP:
    ...
    def __init__(self, dataset, bfgs_iter=100, debug=False):
        ...
        self.parallel = {'max_workers': 5, 'forward':True}
    ...
    def train(self):
        ...
        try:
            result = fmin_l_bfgs_b_parallel(func=self.neg_log_likelihood, x0=theta0, approx_grad=True, maxiter=self.bfgs_iter, m=100, iprint=self.debug, parallel=self.parallel)
            self.theta = result[0]
            self.loss = result[1][0]
        except np.linalg.LinAlgError:
            print('GP. Increase noise term and re-optimization')
            theta0 = np.copy(self.theta)
            theta0[0] += np.log(10)
            try:
                result = fmin_l_bfgs_b_parallel(func=self.neg_log_likelihood, x0=theta0, approx_grad=True, maxiter=self.bfgs_iter, m=10, iprint=self.debug, parallel=self.parallel)
                self.theta = result[0]
                self.loss = result[1][0]
            except:
